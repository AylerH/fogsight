...
> **Update**: Now supports multiple LLMs, including Google Gemini and Alibaba Cloud Qwen 2.5!
...
* An LLM API key. We support the following models:
  * Google Gemini 2.5 Pro
  * Alibaba Cloud Qwen 2.5
  * OpenAI-compatible APIs (GPT-4, Claude, etc.)
...
3. **Configure your API key**
   ...
   Copy `demo-credentials.json` to `credentials.json` and edit it:

   ```bash
   cp demo-credentials.json credentials.json
   ```

   Edit `credentials.json` based on the model you want to use:
